  ...
  <v-flex>
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="showDialog" width="500">
        ...
        <v-card>
          <v-card-title class="headline grey lighten-2" primary-title>Add Delegate</v-card-title>
          <v-layout ma-3>
            <v-form>
              <v-flex>
                <v-select required v-model="typeToSet" :items="delegateType" label="Select a Type"></v-select>
                <v-text-field
                  v-model="delegateToSet"
                  label="Input The address of the delegate you want to add"
                  required
                ></v-text-field>
              </v-flex>
            </v-form>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue" flat v-on:click="add">Add</v-btn>
            <v-btn color="red" flat v-on:click="reset">Cancel</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-toolbar>
          ...
          <v-card-text class="grey lighten-3">
            <v-list>
              <v-list-tile v-if="delegatesToShow[item].length === 0">
                <v-list-tile-content>No delegates yet</v-list-tile-content>
              </v-list-tile>
              <v-list-tile v-else v-for="delegate in delegatesToShow[item]" :key="delegate">
                <v-list-tile-content>{{delegate}}</v-list-tile-content>
                <v-list-tile-action>
                  <v-btn
                    :loading="loading && (clicked === delegate)"
                    color="error"
                    v-on:click="revoke(delegate, item)"
                  >Revoke</v-btn>
                </v-list-tile-action>
              </v-list-tile>
            </v-list>
...
export default {
  data: () => ({
    delegateType: ["delegatesMgmt", "statusRegMgmt", "tcmMgmt"],
    showDialog: false,
    typeToSet: null,
    delegateToSet: null,
    clicked: null,
    ...
    revoke: async function(delegateToRevoke, typeToRevoke) {
      if (
        this.$store.getters.hasPermission(
          this.$store.state.web3.address,
          "delegatesMgmt"
        )
      ) {
        submitRevokeDelegate({
          identity: this.$store.state.identity,
          permission: this.$store.state.contracts[
            this.mapTypeToContract(typeToRevoke)
          ], // select the correct smart contract depending on the typeToSet,
    ...
    add: async function() {
      if (
        ...
        submitAddDelegate({
          identity: this.$store.state.identity,
          permission: this.$store.state.contracts[
            this.mapTypeToContract(this.typeToSet)
          ], // select the correct smart contract depending on the typeToSet,
